...
ANPR_IMGSZ = 1024
ANPR_IOU = 0.45
ANPR_CONF_CANDIDATES = [0.35, 0.30, 0.25, 0.20, 0.18, 0.15, 0.12, 0.10]
MAX_PLATE_CHARS = 11
ROW_THRESHOLD = 0.12
...
if _TRANSFORMERS_AVAILABLE:
    try:
        ...
        vlm_model = (
            ...
            .from_pretrained(VLM_MODEL_ID, torch_dtype=DTYPE, trust_remote_code=True)
            .to(DEVICE)
        )
        vlm_model.eval()
        VLM_AVAILABLE = True
...
HELMET_NOT_WORN = [
    r"no helmet", r"without (?:a )?helmet", r"not wearing (?:a )?helmet",
    r"bare head", r"bareheaded", r"exposed head",
]
NO_SEATBELT = [
    r"no seat ?belt", r"without (?:a )?seat ?belt",
    r"not wearing (?:a )?seat ?belt", r"unfastened seat ?belt",
]
TRIPLE_RIDING = [
    r"three (?:people|persons|riders|passengers)",
    r"more than two (?:people|persons|riders|passengers)",
    r"triple riding",
...
def get_vlm_verdict(image_path, detections, yolo_violations):
    ...
    if need_vlm:
        ...
        if not yolo_covers_helmet and has_rider_context and matches_any(caption, HELMET_NOT_WORN):
...
def read_plate_crop(crop_path):
    candidates = [predict_chars_at_conf(crop_path, conf) for conf in ANPR_CONF_CANDIDATES]
    best = max(candidates, key=score_candidate)
    best["threshold_candidates"] = [{"conf_threshold": c["conf_threshold"], "plate_text": c["plate_text"], "char_count": c["char_count"], "ocr_confidence": c["ocr_confidence"]} for c in candidates]
    return best
...
"""
    ...
    (challan_dir / "challan.json").write_text(json.dumps(record, indent=2, default=str), encoding="utf-8")
    return record
...
if not image_paths:
    raise FileNotFoundError(f"No images found in {INPUT_FOLDER.resolve()}")

records, summary_rows = [], []
for idx, image_path in enumerate(image_paths, start=1):
    print(f"[{idx}/{len(image_paths)}] {image_path.name}")
    annotated_img, detections, yolo_violations = run_traffic_yolo(image_path)
    print(f"  YOLO detections: {[d['class_name'] for d in detections] or 'none'}")
    print(f"  YOLO hints: {yolo_violations or 'none'}")
    vlm_result = get_vlm_verdict(image_path, detections, yolo_violations)
    print(f"  VLM verdict: {vlm_result['verdict']}")
    print(f"  Violations: {vlm_result['violations'] or 'none'}")
    record = write_challan(image_path, annotated_img, detections, vlm_result)
    records.append(record)
    print(f"  Plate: {record['registration_number']}")
    print(f"  Saved: {record['challan_dir']}")
    summary_rows.append({"file": image_path.name, "verdict": record["verdict"], "violations": "; ".join(record["violations"]) if record["violations"] else "-", "registration_number": record["registration_number"], "fine_total": record["fine_total"], "challan_dir": Path(record["challan_dir"]).name})

summary_df = pd.DataFrame(summary_rows)
summary_csv = OUTPUT_FOLDER / "challan_summary.csv"
summary_json = OUTPUT_FOLDER / "challan_summary.json"
summary_df.to_csv(summary_csv, index=False)
summary_json.write_text(json.dumps(records, indent=2, default=str), encoding="utf-8")
display(summary_df)
print(f"Summary CSV : {summary_csv}")
print(f"Summary JSON: {summary_json}")
print(f"Images processed: {len(summary_rows)}")
print(f"Challans issued : {(summary_df['verdict'] == 'VIOLATION').sum()}")
print(f"Total fine      : INR {int(summary_df['fine_total'].sum())}")


# In[ ]:


challan_dirs = sorted([p for p in OUTPUT_FOLDER.iterdir() if p.is_dir() and p.name.startswith("challan_")])
if challan_dirs:
    sample = challan_dirs[0]
    print(f"Preview: {sample}")
    marked = sample / "yolo_marked.jpg"
    if marked.exists():
        display(IPImage(filename=str(marked), width=760))
    plate_ocr = sample / "plate_ocr.jpg"
    if plate_ocr.exists():
        display(IPImage(filename=str(plate_ocr), width=520))
    md_path = sample / "challan.md"
    if md_path.exists():
